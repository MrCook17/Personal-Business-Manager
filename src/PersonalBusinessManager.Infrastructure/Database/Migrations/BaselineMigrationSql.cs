...
// This is the immutable FluentMigrator representation of the approved
// version-13 bootstrap in docs/personal_business_management_application_schema.sql.
// Parity is enforced by BaselineMigrationDefinitionTests.
internal static class BaselineMigrationSql
{
    public const string ConfigureSession =
        """
    ...
    public const string Migration0001 =
        """
        CREATE TABLE `users` (
    ...
    public const string Migration0002 =
        """
    ...
    public const string Migration0003 =
        """
    ...
    public const string Migration0004 =
        """
        CREATE TABLE `jobs` (
            `record_id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
            `customer_id` BIGINT UNSIGNED NOT NULL,
    ...
    public const string Migration0005 =
        """
    ...
    public const string Migration0006 =
        """
        ...
        CREATE TABLE `financial_account_applications` (
            `record_id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
            `user_id` BIGINT UNSIGNED NOT NULL,
            `account_type_id` BIGINT UNSIGNED NOT NULL,
        ...
        CREATE TABLE `financial_account_contributions` (
            `record_id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
            `financial_account_id` BIGINT UNSIGNED NOT NULL,
    ...
    public const string Migration0007 =
        """
    ...
    public const string Migration0008 =
        """
    ...
    public const string Migration0009 =
        """
    ...
    public const string Migration0010 =
        """
        ...
        CREATE TABLE `financial_account_attachments` (
            `record_id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
            `financial_account_id` BIGINT UNSIGNED NOT NULL,
            `attachment_id` BIGINT UNSIGNED NOT NULL,
            `date_created_utc` DATETIME(6) NOT NULL,
        ...
        CREATE TABLE `financial_account_application_attachments` (
            `record_id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
    ...
    public const string Migration0011 =
        """
    ...
    public const string Migration0012 =
        ...
        CREATE INDEX `idx_financial_accounts_maturity_date`
            ON `financial_accounts` (`maturity_date`);
        
        CREATE INDEX `idx_financial_accounts_introductory_end`
            ON `financial_accounts` (`introductory_rate_end_date`);
        
        CREATE INDEX `idx_financial_accounts_provider_name`
            ON `financial_accounts` (`provider_name`);
        
        CREATE INDEX `idx_financial_account_snapshots_account_time`
            ON `financial_account_balance_snapshots` (`financial_account_id`, `balance_at_utc`);
        
        CREATE INDEX `idx_financial_account_snapshots_balance_time`
            ON `financial_account_balance_snapshots` (`balance_at_utc`);
        
        CREATE INDEX `idx_financial_account_applications_status_action`
            ON `financial_account_applications` (`application_status_code`, `next_action_date`);
        
        CREATE INDEX `idx_financial_account_applications_provider`
            ON `financial_account_applications` (`provider_name`);
        
        CREATE INDEX `idx_financial_account_applications_application_date`
            ON `financial_account_applications` (`application_date`);
        
        CREATE INDEX `idx_financial_account_contributions_account_date`
            ON `financial_account_contributions` (`financial_account_id`, `contribution_date`);
        
        CREATE INDEX `idx_financial_account_contributions_tax_year_date`
        ...
        CREATE INDEX `idx_invoices_credit_for_invoice_id`
            ON `invoices` (`credit_for_invoice_id`);
        
        CREATE INDEX `idx_invoice_lines_source_job_id`
            ON `invoice_lines` (`source_job_id`);
        
        CREATE INDEX `idx_invoice_lines_credit_for_line_id`
            ON `invoice_lines` (`credit_for_invoice_line_id`);
        
        CREATE INDEX `idx_invoice_time_entries_invoice_line_id`
        ...
        CREATE INDEX `idx_invoice_payments_account_date`
        ...
        CREATE INDEX `idx_expenses_account_date`
            ON `expenses` (`paid_from_account_id`, `expense_date`);
        
        CREATE INDEX `idx_expenses_supplier_name`
            ON `expenses` (`supplier_name`);
        
        CREATE INDEX `idx_attachments_sha256_hash`
            ON `attachments` (`sha256_hash`);
        
        CREATE INDEX `idx_attachments_date_created`
            ON `attachments` (`date_created_utc`);
        
        CREATE INDEX `idx_customer_attachments_attachment_id`
            ON `customer_attachments` (`attachment_id`);
        
        CREATE INDEX `idx_job_attachments_attachment_id`
            ON `job_attachments` (`attachment_id`);
        
        CREATE INDEX `idx_expense_attachments_attachment_id`
            ON `expense_attachments` (`attachment_id`);
        
        CREATE INDEX `idx_financial_account_attachments_attachment_id`
            ON `financial_account_attachments` (`attachment_id`);
        
        CREATE INDEX `idx_financial_account_application_attachments_attachment_id`
            ON `financial_account_application_attachments` (`attachment_id`);
        """;

    public const string Migration0013 =
        """
    ...
    public const string Migration0012Down =
        """
        DROP INDEX `idx_financial_account_application_attachments_attachment_id` ON `financial_account_application_attachments`;
        DROP INDEX `idx_financial_account_attachments_attachment_id` ON `financial_account_attachments`;
        DROP INDEX `idx_expense_attachments_attachment_id` ON `expense_attachments`;
        DROP INDEX `idx_job_attachments_attachment_id` ON `job_attachments`;
        DROP INDEX `idx_customer_attachments_attachment_id` ON `customer_attachments`;
        DROP INDEX `idx_attachments_date_created` ON `attachments`;
        DROP INDEX `idx_attachments_sha256_hash` ON `attachments`;
        DROP INDEX `idx_expenses_supplier_name` ON `expenses`;
        DROP INDEX `idx_expenses_account_date` ON `expenses`;
        DROP INDEX `idx_expenses_category_date` ON `expenses`;
        DROP INDEX `idx_expenses_expense_date` ON `expenses`;
        DROP INDEX `idx_invoice_payments_account_date` ON `invoice_payments`;
        DROP INDEX `idx_invoice_payments_invoice_date` ON `invoice_payments`;
        DROP INDEX `idx_invoice_time_entries_invoice_line_id` ON `invoice_time_entries`;
        DROP INDEX `idx_invoice_lines_credit_for_line_id` ON `invoice_lines`;
        DROP INDEX `idx_invoice_lines_source_job_id` ON `invoice_lines`;
        DROP INDEX `idx_invoices_credit_for_invoice_id` ON `invoices`;
        DROP INDEX `idx_invoices_invoice_date` ON `invoices`;
        DROP INDEX `idx_invoices_status_due_date` ON `invoices`;
        DROP INDEX `idx_invoices_customer_date` ON `invoices`;
        DROP INDEX `idx_financial_account_contributions_tax_year_date` ON `financial_account_contributions`;
        DROP INDEX `idx_financial_account_contributions_account_date` ON `financial_account_contributions`;
        DROP INDEX `idx_financial_account_applications_application_date` ON `financial_account_applications`;
        DROP INDEX `idx_financial_account_applications_provider` ON `financial_account_applications`;
        DROP INDEX `idx_financial_account_applications_status_action` ON `financial_account_applications`;
        DROP INDEX `idx_financial_account_snapshots_balance_time` ON `financial_account_balance_snapshots`;
        DROP INDEX `idx_financial_account_snapshots_account_time` ON `financial_account_balance_snapshots`;
        DROP INDEX `idx_financial_accounts_provider_name` ON `financial_accounts`;
        DROP INDEX `idx_financial_accounts_introductory_end` ON `financial_accounts`;
        DROP INDEX `idx_financial_accounts_maturity_date` ON `financial_accounts`;
        DROP INDEX `idx_financial_accounts_type_status` ON `financial_accounts`;
        DROP INDEX `idx_financial_accounts_user_scope` ON `financial_accounts`;
        DROP INDEX `idx_tasks_title` ON `tasks`;
        DROP INDEX `idx_tasks_priority_due_date` ON `tasks`;
        DROP INDEX `idx_tasks_status_due_date` ON `tasks`;
        DROP INDEX `idx_tasks_job_id` ON `tasks`;
        DROP INDEX `idx_time_entries_start_utc` ON `time_entries`;
        DROP INDEX `idx_time_entries_billable_start` ON `time_entries`;
        DROP INDEX `idx_time_entries_user_start` ON `time_entries`;
        DROP INDEX `idx_time_entries_job_start` ON `time_entries`;
        DROP INDEX `idx_active_timers_job_id` ON `active_timers`;
        DROP INDEX `idx_jobs_title` ON `jobs`;
        DROP INDEX `idx_jobs_date_archived_utc` ON `jobs`;
        DROP INDEX `idx_jobs_priority_due_date` ON `jobs`;
        DROP INDEX `idx_jobs_status_due_date` ON `jobs`;
        DROP INDEX `idx_jobs_customer_status` ON `jobs`;
        DROP INDEX `idx_customer_addresses_postcode` ON `customer_addresses`;
        DROP INDEX `idx_customer_addresses_customer_type` ON `customer_addresses`;
        DROP INDEX `idx_customer_contacts_name` ON `customer_contacts`;
        DROP INDEX `idx_customer_contacts_email_address` ON `customer_contacts`;
        DROP INDEX `idx_customer_contacts_customer_id` ON `customer_contacts`;
        DROP INDEX `idx_customers_active_archived` ON `customers`;
        DROP INDEX `idx_customers_company_name` ON `customers`;
        DROP INDEX `idx_backup_records_completed` ON `backup_records`;
        DROP INDEX `idx_backup_records_status_started` ON `backup_records`;
        DROP INDEX `idx_audit_records_correlation_id` ON `audit_records`;
        DROP INDEX `idx_audit_records_user_occurred` ON `audit_records`;
        DROP INDEX `idx_audit_records_entity_occurred` ON `audit_records`;
        DROP INDEX `idx_password_recovery_codes_user_created` ON `password_recovery_codes`;
        """;
}
